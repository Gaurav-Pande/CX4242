...
def write_data_to_binary_file(item_list, file_name):
    with open(file_name, "wb") as file_object:
        for item in item_list:
            file_object.write(
                pack("<i q", item, item*item))  # ~~~ MODIFY THIS LINE (i) ~~~
...
def get_memory_map_from_binary_file(file_name):
    num_bytes = 21 * 3 * 4  # ~~~ MODIFY THIS LINE (ii) ~~~
...
def parse_memory_map(file_map):
    ...
    for i in range(21):  # ~~~ MODIFY THIS LINE (iii) ~~~
        parsed_values.append(
            # print i, file_map[i * (4*3): i * (4*3) + (4*3)]
            unpack("<i q", file_map[i * 4 * 3 : i * 4 *3+ 4 * 3]))  # ~~~ MODIFY THIS LINE (iv) ~~~
...
with a list of all even integers in the range [1, 42].
...
"""
def warmup():
    item_list = range(2,43,2)  # ~~~ MODIFY THIS LINE (v) ~~~
