...
def partition_classes(X, y, split_attribute, split_val):
    ...
    attr = X[0][split_attribute]
    if isinstance(attr, basestring):# Categorical Split Attribute
        for idx, epi in enumerate(X):
            if epi[split_attribute] == split_val:
                X_left.append(epi)
                y_left.append(y[idx])
            else:
                X_right.append(epi)
    ...
    else:# Numerical Split Attribute
        for idx, epi in enumerate(X):
            if epi[split_attribute] <= split_val:
                X_left.append(epi)
                y_left.append(y[idx])
...
def information_gain(previous_y, current_y):
    ...
    info_gain = 0.45915
    """
    prev_en = entropy(previous_y)
    p0 = float(len(current_y[0]))/float(len(current_y[0])+len(current_y[1]))
    p1 = 1-p0
    curr_en = p0*entropy(current_y[0]) + p1*entropy(current_y[1])
    info_gain = prev_en - curr_en
