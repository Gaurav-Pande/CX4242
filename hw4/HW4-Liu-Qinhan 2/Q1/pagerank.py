...
(3) unpack the source ID and source degree from the index_map.
...
def pagerank(index_file, edge_file, max_node_id, edge_count, damping_factor=0.85, iterations=10):
    index_map = mmap.mmap(
        index_file.fileno(),
        length=(max_node_id+1) * (8*2),  # ~~~ MODIFY THIS LINE (i) ~~~
        access=mmap.ACCESS_READ)

    edge_map = mmap.mmap(
        edge_file.fileno(),
        length=edge_count * (4*2),  # ~~~ MODIFY THIS LINE (ii) ~~~
        access=mmap.ACCESS_READ)
    ...
    for it in range(iterations):
        ...
        for i in xrange(edge_count):
            ...
            source, target = unpack(
                '<i i',  # ~~~ MODIFY THIS LINE (iii) ~~~
                edge_map[i * (4*2): i * (4*2) + (4*2)])  # ~~~ MODIFY THIS LINE (iv) ~~~

            source_degree = unpack(
                '<q q',  # ~~~ MODIFY THIS LINE (v) ~~~
                index_map[source * (8*2): source * (8*2) + 8*2])[1]  # ~~~ MODIFY THIS LINE (vi) ~~~

            # print "i: ", i, source, target, source_degree
